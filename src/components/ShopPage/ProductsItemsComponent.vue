  ...
  <section class="">
    ...
    >
      <div class="relative">
        ...
        >
          {{ item.Qty === 0 ? $t('out_of_stock') : item.Qty < 5 ? $t('low_stock') : $t('in_stock') }}
        </div>
      </div>
      ...
      <div class="px-3">
        <div class="flex-1">
          <div>
            <h2 class="font-semibold text-gray-700 h-15 text-lg my-2">{{ $t(`products.${item.name}`) }}</h2>
            <p class="text-gray-500 text-sm my-6">{{ $t(`products.${item.categories}`) }}</p>
          </div>
          <div class="flex items-center justify-between mb-2">
            <p class="text-blue-600 text-lg font-semibold">${{ item.price }}</p>
            <p class="text-gray-500 text-xs">{{ $t('qty') }} : {{ item.Qty }}</p>
          </div>
        </div>
      </div>
    </div>
